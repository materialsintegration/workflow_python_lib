...
def get_rundetail(token, url, siteid, runid, port="50443", with_result=False, tool_names=None, debug=False, version="v3"):
    '''
    ラン詳細の取得
    ...
    weburl = "https://%s:%s/workflow-api/%s/runs/%s"%(url, port, version, runid)
    ...
    if len(tool_names) != 0:
        for tool_name in tool_names:
            ...
            weburl = "https://%s:%s/workflow-api/%s/runs/%s/tools?tool=%s"%(url, port, version, runid, tool_name)
            sys.stderr.write("%s\n"%weburl)
            res = mintWorkflowAPI(token, weburl)
            if res.text != "":
    ...
    weburl = "https://%s:%s/workflow-api/%s/runs/%s/data"%(url, port, version, runid)
